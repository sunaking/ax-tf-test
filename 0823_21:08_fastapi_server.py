...
AZURE_ENDPOINT_BASE = (os.environ.get("AZURE_OPENAI_ENDPOINT", "https://azure-openai-price01.openai.azure.com") or "").rstrip("/")
...
def _maybe_llm():
    if not AZURE_API_KEY or not AZURE_ENDPOINT_BASE:
        return None
...
def answer_question(req: QARequest):
    ...
    pairs = (search_with_scores("md", req.question, req.k) + search_with_scores("csv", req.question, req.k)) if kb == "all" else search_with_scores(kb, req.question, req.k)
...
DB_PATH = os.environ.get("PRICE_DB", os.path.join(BASE_DIR, "data", "azure_price_data.db"))
...
def options(category: str = Query(...), region: Optional[str] = None):
    ...
    try:
        if category == "VM":
            q = f"SELECT DISTINCT OS, Cores, MemoryGB, ReservationTerm FROM {TABLE_VM} WHERE Location='koreacentral'"
        ...
        if category == "Disk":
            q = f"SELECT DISTINCT DiskCategory, Tier, Provisioned_GiB, Provisioned_IOPS, ReservationTerm FROM {TABLE_DISK} WHERE Location='koreacentral'"
            df = pd.read_sql_query(q, conn)
            return {
        ...
        if category == "DB":
            q = f"SELECT DISTINCT serviceName, skuName, reservationTerm FROM {TABLE_DB} WHERE armRegionName='koreacentral'"
            df = pd.read_sql_query(q, conn)
            return {
        ...
        if category == "Databricks":
            q = f"SELECT DISTINCT metername FROM {TABLE_DBR}"
            df = pd.read_sql_query(q, conn)
            return {"metername": sorted([x for x in df["metername"].dropna().unique().tolist() if str(x).strip()])}

        if category == "AOAI":
            q = f"SELECT DISTINCT productname, metername FROM {TABLE_AOAI}"
            df = pd.read_sql_query(q, conn)
            return {
                "productname": sorted([x for x in df["productname"].dropna().unique().tolist() if str(x).strip()]),
                "metername": sorted([x for x in df["metername"].dropna().unique().tolist() if str(x).strip()])
            }
...
def items(
    ...
    try:
        ...
        # ---------- VM ----------
        if category == "VM":
            q = f"SELECT * FROM {TABLE_VM} WHERE Location='koreacentral'"
            cond, args = [], []
        ...
        # ---------- Disk ----------
            ...
            q = f"SELECT * FROM {TABLE_DISK} WHERE Location='koreacentral'"
            cond, args = [], []
        ...
        # ---------- DB ----------
            ...
            q = f"SELECT * FROM {TABLE_DB} WHERE armRegionName='koreacentral'"
            cond, args = [], []
...
def diag_vm():
    conn = _conn()
    try:
        q = f"SELECT RetailPrice, UnitOfMeasure, SKUName, Location, OS, Cores, MemoryGB FROM {TABLE_VM} WHERE Location='koreacentral'"
        df = pd.read_sql_query(q, conn)
        total = len(df)
