...
.right-panel .accent-input .stNumberInput input{
  background:#E6F4F1 !important;            /* 요청 배경색 */
...
.export-area .stDownloadButton > button {
  ...
  color:#000 !important;               /* 글자 검정 */
  border:none !important;
  border-radius:12px !important;
  height:46px !important;
  font-weight:700 !important;
}
/* 삭제 버튼은 기본 스타일 유지(색 지정 안 함) */
...
if mode == "장바구니":
    ...
    with st.sidebar:
        ...
        if sel_cat == "VM":
            ...
            p_mn = st.selectbox("metername", [""] + opts.get("metername", [])) or None
            if p_mn: params["metername"] = p_mn

        elif sel_cat == "AOAI":
            opts = api_options("AOAI", sel_region)
            p_pn = st.selectbox("productname", [""] + opts.get("productname", [])) or None
            if p_pn: params["productname"] = p_pn
            p_mn = st.selectbox("metername", [""] + opts.get("metername", [])) or None
            if p_mn: params["metername"] = p_mn
    ...
    if df.empty:
        st.info("조건에 맞는 항목이 없습니다. 좌측 필터를 조정해 보세요.")
        st.stop()
    ...
    with right:
        ...
        unit_month_usd = float(sel.get("price_per_month_after_rule", sel.get("price_per_month", 0.0)))
        ...
        if st.button("장바구니 담기", use_container_width=True):
            row = {
                ...
                "unit_price_month_usd": round(unit_month_usd, 6),  # 내부 계산용
                "qty": int(qty),
                "months": int(months),
